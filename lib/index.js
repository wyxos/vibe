import { nextTick as O, ref as F, computed as Q, defineComponent as Xe, onMounted as Ge, onUnmounted as qe, watch as te, createElementBlock as U, openBlock as _, createCommentVNode as ae, createElementVNode as G, normalizeStyle as he, renderSlot as J, normalizeClass as ie, withModifiers as Re, toDisplayString as Ee, unref as q, Fragment as Ce, renderList as Oe, createVNode as Le, withCtx as ve, mergeProps as fe, TransitionGroup as Pt } from "vue";
let ke = null;
function It() {
...
function Ae(e, l, i, a = {}) {
  ...
  try {
    if (l && l.nodeType === 1 && typeof window < "u" && window.getComputedStyle) {
      ...
      T = parseFloat(L.paddingLeft) || 0, y = parseFloat(L.paddingRight) || 0;
    }
  } catch {
  }
  const h = (s || 0) + T, P = (w || 0) + y, I = l.offsetWidth - l.clientWidth, f = I > 0 ? I + 2 : It() + 2, r = l.offsetWidth - f - h - P, u = g * (i - 1), k = Math.floor((r - u) / i), M = e.map((L) => {
    const B = L.width, R = L.height;
    return Math.round(k * R / B) + o + v;
  });
  if (x === "sequential-balanced") {
    const L = M.length;
    if (L === 0) return [];
    const B = (b, E, C) => b + (E > 0 ? d : 0) + C;
    let R = Math.max(...M), $ = M.reduce((b, E) => b + E, 0) + d * Math.max(0, L - 1);
    const W = (b) => {
      let E = 1, C = 0, ne = 0;
      for (let X = 0; X < L; X++) {
        const Z = M[X], K = B(C, ne, Z);
        if (K <= b)
          C = K, ne++;
        else if (E++, C = Z, ne = 1, Z > b || E > i) return !1;
      }
      return E <= i;
    };
    for (; R < $; ) {
    ...
    for (let b = L - 1; b >= 0; b--) {
      const E = M[b], C = b < p;
      !(B(S, z, E) <= H) || C ? (n[p] = b + 1, p--, S = E, z = 1) : (S = B(S, z, E), z++);
    }
    n[0] = 0;
    const V = [], Y = new Array(i).fill(0);
    for (let b = 0; b < i; b++) {
      const E = n[b], C = b + 1 < i ? n[b + 1] : L, ne = b * (k + g);
      for (let X = E; X < C; X++) {
        const K = {
          ...e[X],
          columnWidth: k,
        ...
        K.imageHeight = M[X] - (o + v), K.columnHeight = M[X], K.left = ne, K.top = Y[b], Y[b] += K.columnHeight + (X + 1 < C ? d : 0), V.push(K);
      }
    }
    return V;
  }
  ...
  for (let L = 0; L < e.length; L++) {
    ...
    R.columnWidth = k, R.left = $ * (k + g), R.imageHeight = Math.round(k * H / W), R.columnHeight = R.imageHeight + o + v, R.top = m[$], m[$] += R.columnHeight + d, N.push(R);
  }
  return N;
}
...
function Ue(e, l, i) {
  ...
  function y(m) {
    var N = a, L = g;
    return a = g = void 0, w = m, v = e.apply(L, N), v;
  }
  function h(m) {
    return w = m, o = setTimeout(f, l), t ? y(m) : v;
  }
  function P(m) {
    var N = m - s, L = m - w, B = l - N;
    return x ? Jt(B, d - L) : B;
  }
  function I(m) {
    var N = m - s, L = m - w;
    return s === void 0 || N >= l || N < 0 || x && L >= d;
  }
  function f() {
    var m = Se();
    if (I(m))
...
function ta(e, l) {
  ...
  function w(t, x) {
    ...
    if (!Number.isFinite(I)) {
      const N = getComputedStyle(t).getPropertyValue("--masonry-leave-duration") || "", L = parseFloat(N);
      I = Number.isFinite(L) && L > 0 ? L : 200;
    }
    const f = t.style.transitionDuration, r = () => {
      t.removeEventListener("transitionend", u), clearTimeout(k), t.style.transitionDuration = f || "";
    }, u = (m) => {
      (!m || m.target === t) && (r(), x());
    }, k = setTimeout(() => {
      r(), x();
...
function la(e) {
  ...
  async function N(H) {
    try {
      ...
      return a.value = p, await O(), await O(), await O(), w(p), n;
    } catch (n) {
      throw n;
    }
  }
  async function L(H, n = !1) {
    ...
    if (!(a.value.length >= p)) {
      ...
      try {
        ...
        for (f("backfill:start", {
          ...
          const C = o.value, ne = s.value[s.value.length - 1];
          if (await M(h, (Z, K) => {
            f("backfill:tick", {
              fetched: a.value.length,
              target: p,
  ...
  async function B(H) {
    if (!g.value) {
      ...
      try {
        ...
        if (r.value) return;
        const p = await N(H);
        return r.value ? void 0 : (v.value = null, o.value = H, s.value.push(p.nextPage), p.nextPage == null && (d.value = !0), await L(n), p);
      } catch (n) {
  ...
  async function R() {
    if (!g.value && !d.value) {
      ...
      try {
        ...
        if (r.value) return;
        if (y === "refresh" && o.value != null && k(o.value) < I) {
          const z = await m(() => l(o.value, i == null ? void 0 : i.value));
          if (r.value) return;
          const V = [...a.value], Y = z.items.filter((E) => !E || E.id == null || E.page == null ? !1 : !V.some((C) => C && C.id === E.id && C.page === E.page));
          if (Y.length > 0) {
            const E = [...a.value, ...Y];
            a.value = E, await O(), await O(), await O(), w(E);
          }
          if (v.value = null, Y.length === 0) {
            ...
            const C = await N(E);
            return r.value ? void 0 : (v.value = null, o.value = E, s.value.push(C.nextPage), C.nextPage == null && (d.value = !0), await L(H), C);
          }
          if (k(o.value) >= I) {
            const E = s.value[s.value.length - 1];
            if (E == null) {
        ...
        const p = await N(n);
        return r.value ? void 0 : (v.value = null, o.value = n, s.value.push(p.nextPage), p.nextPage == null && (d.value = !0), await L(H), p);
      } catch (H) {
...
function oa(e) {
  ...
  async function T(f) {
    ...
    const r = new Set(f.map((k) => k.id)), u = l.value.filter((k) => !r.has(k.id));
    if (l.value = u, await O(), u.length === 0 && v.value.length > 0) {
      try {
        await g(), await d(0, !0);
      } catch {
      }
  ...
  async function h(f, r) {
    ...
    M.splice(m, 0, f), l.value = M, await O(), i.value || (await O(), await O(), a(M));
  }
  async function P(f, r) {
    ...
    const u = l.value, k = new Set(u.map((W) => W.id)), M = [];
    for (let W = 0; W < f.length; W++)
      k.has(($ = f[W]) == null ? void 0 : $.id) || M.push({ item: f[W], index: r[W] });
    if (M.length === 0) return;
    const m = /* @__PURE__ */ new Map();
    for (const { item: W, index: H } of M)
      m.set(H, W);
    const N = M.length > 0 ? Math.max(...M.map(({ index: W }) => W)) : -1, L = Math.max(u.length - 1, N), B = [];
    let R = 0;
    for (let W = 0; W <= L; W++)
      m.has(W) ? B.push(m.get(W)) : R < u.length && (B.push(u[R]), R++);
    for (; R < u.length; )
      B.push(u[R]), R++;
    l.value = B, await O(), i.value || (await O(), await O(), a(B));
  }
  async function I() {
    l.value = [];
  }
...
function ra(e) {
  ...
  function T(P) {
    ...
    if (w(P, "refreshLayout"), P.length > 1e3 && t.length > P.length && t.length - P.length < 100) {
      ...
      for (let m = 0; m < P.length; m++)
        if (((u = P[m]) == null ? void 0 : u.id) !== ((k = t[m]) == null ? void 0 : k.id)) {
          M = !1;
          break;
        }
      if (M) {
  ...
  function y(P, I) {
    s.value = P, P && (P.width !== void 0 && (d.value = P.width), !i.value && a.value && l.value.length > 0 && O(() => {
  ...
  const {
    ...
    return !r || r.length === 0 ? [] : r.filter((k) => {
      if (typeof k.top != "number" || typeof k.columnHeight != "number")
        return !0;
      const M = k.top;
      return k.top + k.columnHeight >= I && M <= f;
    });
  });
  function y(I) {
    if (!i.value) return;
    const { scrollTop: f, clientHeight: r } = i.value, u = f + r, k = I ?? $e(l.value, a.value), M = k.length ? Math.max(...k) : 0, m = typeof d == "number" ? d : 200, N = m >= 0 ? Math.max(0, M - m) : Math.max(0, M + m), L = Math.max(0, N - u), B = L <= 100;
    x.value = {
...
const ua = { class: "flex-1 relative min-h-0" }, ca = { class: "w-full h-full rounded-xl overflow-hidden shadow-sm transition-all duration-300 bg-white relative" }, va = {
  ...
  setup(e, { emit: l }) {
    ...
    function B(n) {
      const p = n.target;
      p && (k.value || p.play(), m("video"));
    }
    function R(n) {
      const p = n.target;
      p && (k.value || p.pause(), N("video"));
    }
    function $(n) {
      return new Promise((p, S) => {
        ...
        z.onload = () => {
          ...
          setTimeout(async () => {
            g.value = !0, d.value = !1, y.value = !1, await O(), await new Promise((C) => setTimeout(C, 100)), h.value = !0, a("preload:success", { item: i.item, type: "image", src: n }), M("image", n), p();
          }, E);
        }, z.onerror = () => {
    ...
    function W(n) {
      return new Promise((p, S) => {
        ...
        z.preload = "metadata", z.muted = !0, z.onloadedmetadata = () => {
          const b = Date.now() - V, E = Math.max(0, Y - b);
          setTimeout(async () => {
            o.value = !0, s.value = !1, y.value = !1, await O(), await new Promise((C) => setTimeout(C, 100)), h.value = !0, a("preload:success", { item: i.item, type: "video", src: n }), M("video", n), p();
          }, E);
        }, z.onerror = () => {
    ...
    return Ge(async () => {
        ...
        (S) => {
          S.forEach((z) => {
            ...
            if (Y && !x.value) {
              ...
              const E = r.value === "image" ? v.value : w.value, C = r.value === "image" ? g.value : o.value;
              E && C && M(r.value, E);
            }
      ...
      ), f.observe(P.value), await O(), await O(), await O(), p(), setTimeout(() => {
        p();
      }, 100);
      function p() {
      ...
      () => i.isActive,
      (n) => {
        !k.value || !I.value || (n ? I.value.play() : I.value.pause());
      }
    ), (n, p) => (_(), U("div", {
  ...
  setup(e, { expose: l, emit: i }) {
    ...
    }), r = F(7), u = F(null), k = F([]), M = F(null), m = F(!1), N = F(0), L = F(!1), B = F(null), R = F(!1), $ = Q(() => Kt(o.value)), W = sa({
      masonry: h
    ...
    }), { viewportTop: b, viewportHeight: E, virtualizing: C, visibleMasonry: ne, updateScrollProgress: X, updateViewport: Z, reset: K } = Y, { onEnter: Qe, onBeforeEnter: Ze, onBeforeLeave: et, onLeave: tt } = ta(
      { container: u },
    ...
    function bt() {
      Me(), u.value && u.value.scrollTo({
        top: 0,
        behavior: "smooth"
      }), h.value = [], s.value = 0, M.value = a.loadAtPage, k.value = [a.loadAtPage], L.value = !1, B.value = null, R.value = !1, K();
    }
    function Mt() {
      Me(), h.value = [], N.value = 0, M.value = null, k.value = [], L.value = !1, B.value = null, m.value = !1, R.value = !1, ce.value = 0, pe.value = 0, ye.value = !1, K(), n(), u.value && u.value.scrollTo({
        top: 0,
        behavior: "auto"
    ...
    function Tt(c, D, j) {
      M.value = D, k.value = [D], j != null && k.value.push(j), L.value = j === null, H(c, "initialize");
      const le = h.value, A = le.length === 0 ? c : [...le, ...c];
      h.value = A, T.value ? ce.value === 0 && h.value.length > 0 && (pe.value = 0) : (S(A), u.value && (b.value = u.value.scrollTop, E.value = u.value.clientHeight || window.innerHeight), O(() => {
    ...
    return te(
      ...
      T.value && c > 0 && D === 0 && (ce.value = 0, O(() => Ie()));
    }), te(v, (c) => {
      ...
      c !== D && c > 0 && !T.value && u.value && h.value.length > 0 && O(() => {
        r.value = se(d.value, c), S(h.value), X();
      });
    }), Ge(async () => {
      try {
        await O(), v.value && !t && (o.value = v.value.clientWidth, s.value = v.value.clientHeight), T.value || (r.value = se(d.value, o.value), u.value && (b.value = u.value.scrollTop, E.value = u.value.clientHeight));
        const c = a.loadAtPage;
        if (k.value = [c], a.init === "auto") {
          R.value = !0, await O();
          try {
            await be(c);
          } catch {
      ...
      R.value ? T.value ? (_(), U("div", {
        ...
        L.value && h.value.length > 0 ? (_(), U("div", ba, [
        ...
        L.value && h.value.length > 0 ? (_(), U("div", Pa, [
          J(c.$slots, "end-message", {}, () => [
  ...
  for (const [a, g] of l)
    i[a] = g;
  return i;
}, _e = /* @__PURE__ */ Sa(ka, [["__scopeId", "data-v-56ba77e1"]]), Ha = {
  install(e) {
