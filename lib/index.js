...
function Xe(e, o, r, t = {}) {
  ...
  const u = (w || 0) + p, m = (x || 0) + T, l = o.offsetWidth - o.clientWidth, s = l > 0 ? l + 2 : It() + 2, v = o.offsetWidth - s - u - m, b = y * (r - 1), P = Math.floor((v - b) / r), I = e.map((L) => {
...
function Qe(e, o, r) {
  ...
  function u(f) {
    return x = f, i = setTimeout(s, o), a ? T(f) : h;
  }
  function m(f) {
    var $ = f - w, L = f - x, E = o - $;
    return M ? Gt(E, g - L) : E;
  ...
  function I() {
    ...
    if (t = arguments, y = this, w = f, $) {
      if (i === void 0)
        return u(w);
      if (M)
        return clearTimeout(i), i = setTimeout(s, o), T(w);
    }
    return i === void 0 && (i = setTimeout(s, o)), h;
  }
...
function ea(e, o) {
  ...
  function g(a, M) {
    var u;
    const p = (u = e.container) == null ? void 0 : u.value;
    if (p) {
  ...
  function h(a, M) {
    var v;
    const p = parseInt(a.dataset.left || "0", 10), T = parseInt(a.dataset.top || "0", 10), u = parseInt(a.dataset.index || "0", 10), m = a.offsetHeight || parseInt(getComputedStyle(a).height || "200", 10) || 200;
    ...
    const l = Math.min(u * 20, 160), s = a.style.getPropertyValue("--masonry-opacity-delay");
    a.style.setProperty("--masonry-opacity-delay", `${l}ms`), requestAnimationFrame(() => {
      ...
      const b = () => {
        s ? a.style.setProperty("--masonry-opacity-delay", s) : a.style.removeProperty("--masonry-opacity-delay"), a.removeEventListener("transitionend", b), M();
      };
  ...
  function x(a, M) {
    var I;
    const p = parseInt(a.dataset.left || "0", 10), T = parseInt(a.dataset.top || "0", 10), u = a.offsetHeight || parseInt(getComputedStyle(a).height || "200", 10) || 200;
    ...
    const s = a.style.transitionDuration, v = () => {
      a.removeEventListener("transitionend", b), clearTimeout(P), a.style.transitionDuration = s || "";
    }, b = (f) => {
      (!f || f.target === a) && (v(), M());
    }, P = setTimeout(() => {
      v(), M();
...
function ta({
  ...
  async function M(p, T = !1) {
    if (!e.value) return;
    const u = p ?? be(o.value, r.value), m = u.length ? Math.max(...u) : 0, l = e.value.scrollTop + e.value.clientHeight, s = e.value.scrollTop > a + 1;
    a = e.value.scrollTop;
...
function aa(e) {
  ...
  function I(S) {
    ...
    Math.abs(n) > 100 ? n > 0 && m.value ? v() : n < 0 && u.value ? s() : l() : l(), S.preventDefault();
  }
  function f(S) {
...
function na(e) {
  ...
  async function I(n) {
    ...
    for (; ; )
      try {
        const F = await n();
        return d > 0 && s("retry:stop", { attempt: d, success: !0 }), F;
      } catch (F) {
        if (d++, d > k)
          throw s("retry:stop", { attempt: d - 1, success: !1 }), F;
        s("retry:start", { attempt: d, max: k, totalMs: z }), await P(z, (O, A) => {
          s("retry:tick", { attempt: d, remainingMs: O, totalMs: A });
        }), z += M;
      }
  }
  async function f(n) {
  ...
  async function $(n, d = !1) {
    ...
    if (!(r.value.length >= k)) {
      ...
      try {
        let F = 0;
        for (s("backfill:start", { target: k, fetched: r.value.length, calls: F }); r.value.length < k && F < u && i.value[i.value.length - 1] != null && !v.value && !y.value && b && (await P(T, (A, G) => {
          s("backfill:tick", {
        ...
        s("backfill:stop", { fetched: r.value.length, calls: F });
      } finally {
        b = !1, t.value = !1, s("loading:stop", { fetched: r.value.length });
      }
    }
  }
  async function L(n) {
  ...
  async function E() {
    if (!t.value && !y.value) {
      ...
      try {
        ...
        if (d == null) {
          y.value = !0, t.value = !1, s("loading:stop", { fetched: r.value.length });
          return;
        }
        const k = await f(d);
        ...
        t.value = !1, s("loading:stop", { fetched: r.value.length });
      }
    }
  }
  async function R() {
  ...
  function S() {
    const n = b;
    v.value = !0, t.value = !1, b = !1, n && s("backfill:stop", { fetched: r.value.length, calls: 0, cancelled: !0 }), s("loading:stop", { fetched: r.value.length });
  }
  return {
    loadPage: L,
    loadNext: E,
...
function la(e) {
  ...
  async function x(u) {
    const m = o.value.filter((l) => l.id !== u.id);
  ...
  async function a(u) {
    ...
    const m = new Set(u.map((s) => s.id)), l = o.value.filter((s) => !m.has(s.id));
  ...
  async function M(u, m) {
    if (!u) return;
    const l = o.value;
    if (l.findIndex((P) => P.id === u.id) !== -1) return;
    const v = [...l], b = Math.min(m, v.length);
    v.splice(b, 0, u), o.value = v, await q(), r.value || (await new Promise((P) => requestAnimationFrame(() => P())), requestAnimationFrame(() => {
      t(v);
    }));
  }
  async function p(u, m) {
    ...
    const l = o.value, s = new Set(l.map((E) => E.id)), v = [];
    for (let E = 0; E < u.length; E++)
      s.has((L = u[E]) == null ? void 0 : L.id) || v.push({ item: u[E], index: m[E] });
...
function oa(e) {
  ...
  function p(m) {
    ...
    if (w.value && w.value.width !== void 0) {
      ...
      const $ = Xe(s, v, y.value, i.value);
      v.style.width = I, v.style.boxSizing = f, M($), o.value = $, a = $;
    } else {
      const I = Xe(s, v, y.value, i.value);
      M(I), o.value = I, a = I;
    }
  }
  function T(m, l) {
...
function ra(e) {
  const {
    ...
    const l = i.value - x, s = i.value + w.value + x, v = o.value;
    return !v || v.length === 0 ? [] : v.filter((P) => {
      if (typeof P.top != "number" || typeof P.columnHeight != "number")
        return !0;
      const I = P.top;
      return P.top + P.columnHeight >= l && I <= s;
    });
  });
  function T(l) {
    if (!r.value) return;
    const { scrollTop: s, clientHeight: v } = r.value, b = s + v, P = l ?? be(o.value, t.value), I = P.length ? Math.max(...P) : 0, f = typeof g == "number" ? g : 200, $ = f >= 0 ? Math.max(0, I - f) : Math.max(0, I + f), L = Math.max(0, $ - b), E = L <= 100;
    M.value = {
  ...
  async function u() {
    if (r.value) {
      const s = r.value.scrollTop, v = r.value.clientHeight || window.innerHeight, b = v > 0 ? v : window.innerHeight;
      i.value = s, w.value = b;
    }
    a.value = !0, await q(), await new Promise((s) => requestAnimationFrame(() => s())), a.value = !1;
...
const sa = { class: "flex-1 relative min-h-0" }, ua = { class: "w-full h-full rounded-xl overflow-hidden shadow-sm transition-all duration-300 bg-white relative" }, ca = {
  ...
  setup(e, { emit: o }) {
    const r = e, t = o, y = N(!1), g = N(!1), h = N(null), i = N(!1), w = N(!1), x = N(null), a = N(!1), M = N(!1), p = N(!1), T = N(!1), u = N(null), m = N(null);
    let l = null;
    const s = Z(() => {
      var n;
    ...
    async function S() {
      var d;
      if (!a.value || p.value || v.value || s.value === "video" && i.value || s.value === "image" && y.value)
        return;
      const n = (d = r.item) == null ? void 0 : d.src;
      if (n)
        if (p.value = !0, T.value = !1, s.value === "video") {
      ...
      l = new IntersectionObserver(
        (d) => {
          d.forEach((k) => {
            ...
            F && !M.value && (M.value = !0, t("in-view", { item: r.item, type: s.value })), O && !a.value ? (a.value = !0, S()) : k.isIntersecting;
          });
        },
        {
        ...
        X(n.$slots, "default", {
          ...
          V("div", ua, [
            v.value ? (j(), C("div", ca, d[3] || (d[3] = [
              ...
              s.value === "video" && x.value ? (j(), C("video", {
                key: 1,
                ref_key: "videoEl",
                ref: m,
                src: x.value,
              ...
              !y.value && !i.value && !g.value && !w.value ? (j(), C("div", {
                ...
                V("div", ha, [
                  X(n.$slots, "placeholder-icon", { mediaType: s.value }, () => [
                    V("i", {
  ...
  setup(e, { expose: o, emit: r }) {
    ...
    const p = Z(() => {
      ...
      const c = typeof t.mobileBreakpoint == "string" ? M(t.mobileBreakpoint) : t.mobileBreakpoint;
      return i.value < c;
    }), T = r, u = Z({
      get: () => t.items,
      set: (c) => T("update:items", c)
    }), m = N(7), l = N(null), s = N([]), v = N(null), b = N(!1), P = N(0), I = N(!1), f = N(null), $ = Z(() => Jt(i.value)), L = ia({
      masonry: u
    ...
    function gt(c) {
      d(c, B), !c && h.value && (i.value = h.value.clientWidth, w.value = h.value.clientHeight);
    }
    o({
      isLoading: b,
      refreshLayout: n,
    ...
    function yt(c) {
      l.value && (l.value.scrollTo({
        top: c.top ?? l.value.scrollTop,
        left: c.left ?? l.value.scrollLeft,
    ...
    function xt() {
      Ie(), l.value && l.value.scrollTo({
        top: 0,
        behavior: "smooth"
      }), u.value = [], w.value = 0, v.value = t.loadAtPage, s.value = [t.loadAtPage], I.value = !1, f.value = null, J(), we = !1;
    }
    function bt() {
      Ie(), u.value = [], P.value = 0, v.value = null, s.value = [], I.value = !1, f.value = null, b.value = !1, se.value = 0, ve.value = 0, ye.value = !1, J(), R(), l.value && l.value.scrollTo({
        top: 0,
        behavior: "auto"
    ...
    function Ve(c, H, D) {
      v.value = H, s.value = [H], s.value.push(D), I.value = D == null, E(c, "init"), p.value ? (u.value = [...u.value, ...c], se.value === 0 && u.value.length > 0 && (ve.value = 0)) : (n([...u.value, ...c]), l.value && (F.value = l.value.scrollTop, O.value = l.value.clientHeight || window.innerHeight), q(() => {
        l.value && (F.value = l.value.scrollTop, O.value = l.value.clientHeight || window.innerHeight, B());
      }));
    }
    async function Le(c, H, D) {
      if (!t.skipInitialLoad) {
        Ve(c, H, D);
        return;
      }
      if (v.value = H, s.value = [H], D != null && s.value.push(D), I.value = D === null, f.value = null, E(c, "restoreItems"), p.value)
        u.value = c, se.value === 0 && u.value.length > 0 && (ve.value = 0);
      else if (n(c), l.value && (F.value = l.value.scrollTop, O.value = l.value.clientHeight || window.innerHeight), await q(), l.value) {
        F.value = l.value.scrollTop, O.value = l.value.clientHeight || window.innerHeight, B(), await q();
        const _ = be(u.value, m.value), W = _.length ? Math.max(..._) : 0, qe = l.value.scrollTop + l.value.clientHeight, Se = typeof t.loadThresholdPx == "number" ? t.loadThresholdPx : 200, Mt = Se >= 0 ? Math.max(0, W - Se) : Math.max(0, W + Se);
        qe >= Mt && !I.value && !b.value && s.value.length > 0 && s.value[s.value.length - 1] != null && await Fe(_, !0);
      }
    }
    te(
      g,
      () => {
        p.value || l.value && (m.value = ue(g.value, i.value), n(u.value));
      },
      { deep: !0 }
    ), te(() => t.layoutMode, () => {
      x.value && x.value.width !== void 0 ? i.value = x.value.width : h.value && (i.value = h.value.clientWidth);
    }), te(l, (c) => {
      c && !p.value ? (c.removeEventListener("scroll", oe), c.addEventListener("scroll", oe, { passive: !0 })) : c && c.removeEventListener("scroll", oe);
    }, { immediate: !0 });
    let we = !1;
    return te(
      () => [t.items, t.skipInitialLoad, t.initialPage, t.initialNextPage],
      ([c, H, D, _]) => {
        if (H && c && c.length > 0 && !we) {
          we = !0;
          const W = D ?? t.loadAtPage;
          Le(c, W, _ !== void 0 ? _ : void 0);
        }
      },
      { immediate: !1 }
    ), te(p, (c, H) => {
      H === void 0 && c === !1 || q(() => {
        c ? (document.addEventListener("mousemove", Ee), document.addEventListener("mouseup", Pe), l.value && l.value.removeEventListener("scroll", oe), se.value = 0, ve.value = 0, u.value.length > 0 && ke()) : (document.removeEventListener("mousemove", Ee), document.removeEventListener("mouseup", Pe), l.value && h.value && (x.value && x.value.width !== void 0 ? i.value = x.value.width : i.value = h.value.clientWidth, l.value.removeEventListener("scroll", oe), l.value.addEventListener("scroll", oe, { passive: !0 }), u.value.length > 0 && (m.value = ue(g.value, i.value), n(u.value), F.value = l.value.scrollTop, O.value = l.value.clientHeight, B())));
      });
    }, { immediate: !0 }), te(le, (c) => {
      c && (c.addEventListener("touchstart", Ae, { passive: !1 }), c.addEventListener("touchmove", We, { passive: !1 }), c.addEventListener("touchend", Oe), c.addEventListener("mousedown", Re));
    }), te(() => u.value.length, (c, H) => {
      p.value && c > 0 && H === 0 && (se.value = 0, q(() => ke()));
    }), te(h, (c) => {
      ...
      }), a.observe(c), x.value || (i.value = c.clientWidth, w.value = c.clientHeight)) : c && (x.value || (i.value = c.clientWidth, w.value = c.clientHeight));
    }, { immediate: !0 }), te(i, (c, H) => {
      c !== H && c > 0 && !p.value && l.value && u.value.length > 0 && q(() => {
        m.value = ue(g.value, c), n(u.value), B();
      });
    }), tt(async () => {
      try {
        ...
        if (s.value = [c], !t.skipInitialLoad)
          await Te(s.value[0]);
      ...
      p.value ? (j(), C("div", {
        ...
        V("div", {
          ...
          style: ge({
            ...
            height: `${u.value.length * 100}%`
          })
        }, [
          (j(!0), C(Ue, null, _e(u.value, (D, _) => (j(), C("div", {
            key: `${D.page}-${D.id}`,
            class: "absolute top-0 left-0 w-full",
            style: ge({
              top: `${_ * (100 / u.value.length)}%`,
              height: `${100 / u.value.length}%`
            })
          }, [
            V("div", pa, [
              V("div", ya, [
                X(c.$slots, "default", {
                  item: D,
                  remove: U(ce),
                  index: D.originalIndex ?? u.value.indexOf(D)
                }, () => [
                  He(Me, {
                    item: D,
                    remove: U(ce),
                    ...
                    header: fe((W) => [
                      X(c.$slots, "item-header", de({ ref_for: !0 }, W), void 0, !0)
                    ]),
                    footer: fe((W) => [
                      X(c.$slots, "item-footer", de({ ref_for: !0 }, W), void 0, !0)
                    ]),
                    _: 2
