...
import { type FeedItem, fetchVideoPage, type PageToken } from '@/demo/fakeServer'
import {
...
const items = ref<FeedItem[]>([])
const initialPageToken = 1

async function getContent(pageToken: PageToken) {
  return fetchVideoPage(pageToken)
}
</scr${'ipt'}>
  ...
  <div class="h-full min-h-0 overflow-hidden">
    ...
    >
      <header class="flex items-center gap-4">
        <div class="min-w-0">
          <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Video-only</h1>
          <p class="text-sm text-slate-600">
            Fake server demo · 100 pages · 20 items/page · videos only
          </p>
        </div>
        ...
        <div class="ml-auto flex flex-wrap items-center justify-end gap-2">
          ...
          >
            ...
            <span data-testid="pages-loaded" class="ml-2 tabular-nums text-slate-900">
              {{ pagesLoadedLabel }}
            </span>
          </span>
        ...
        >
          <div class="flex items-center justify-between gap-3 border-b border-slate-200 px-4 py-3">
            <div class="min-w-0">
              ...
              <div class="text-xs text-slate-500">Video-only</div>
            </div>
