...
import { createVibeAssetLoadQueue } from '@/components/vibe-root/useVibeAssetLoadQueue'

describe('useVibeAssetLoadQueue', () => {
  it('limits concurrent loads globally, by domain, and by video domain cap', () => {
    const queue = createVibeAssetLoadQueue({
      maxGlobal: 10,
      maxPerDomain: 4,
      maxVideoPerDomain: 2,
    })
...
function requestAsset(
  queue: ReturnType<typeof createVibeAssetLoadQueue>,
  granted: string[],
  id: string,
  url: string,
